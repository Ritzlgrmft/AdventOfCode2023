...
namespace AdventOfCode2023.Day22a;
...
public class Worker : IWorker
{
    public long DoWork(string inputFile)
    {
        ...
        var multipleSupportedBricks = bricks
            .Select((b, i) => (brick: b, index: i))
            .Where(bi => supportingBricks.Where(sb => sb.supporting.Contains(bi.index)).Count() >= 2)
            .Select(bi => bi.index)
            .ToList();

        var removableBricks = supportingBricks
            .Where(sb => multipleSupportedBricks.Intersect(sb.supporting).Count() == sb.supporting.Count)
            .Select(sb => sb.brick)
            .ToList();
