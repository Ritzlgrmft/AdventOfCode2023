namespace AdventOfCode2023.Day9b;
...
public class Worker : IWorker
{
    public long DoWork(string inputFile)
    {
        ...
        {
            var lastValue = 0;
            sequence.Last().Insert(0, lastValue);
            for (var i = sequence.Count - 2; i >= 0; i--)
            {
                lastValue = sequence[i].First() - lastValue;
                sequence[i].Insert(0, lastValue);
            }
        }

        return sequences.Sum(s => s[0].First());
    }
}
